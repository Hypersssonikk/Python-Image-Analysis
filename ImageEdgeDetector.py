...
for p in range (4,5):#next line of code is file location on laptop or desktop
    im = Image.open("C:/Users/User/Desktop/New folder/sea" + str(p) + ".jpg") 
    pix=im.load()
    for r in range (1,8): #this block of code generates square blocks made of  smaller square blocks of  pixels  to input pixel values from across image
     for s in range (1,17):
      count=0
      for i in range (1,70,10):
       for j in range (1,70,10): 
        wet=pix [-50+i + s*70,-40 + j + r*70 ] #changing the value of -50 ( positive values too ) allows detector to be placed in desired region of image
        count=count+1
        a=a+wet[0] # one red pixel value
        c=c+wet[1] #one green pixel value
        e=e+wet[2] #one blue pixel value
...
for f in range(1,17):
 for e in range (1,8):
  ...
  for w in range (1,70):
    for v in range(1,70):
     if (array5[(count5 +113)]-array5[count5]>0 or array5[(count5 +113)]-array5[count5])<0 :
      im.putpixel([-50 + v+ f*70,-50+w +e*70],(255,255,255))
